"""
SOURCE: https://github.com/mpchang/uncovering-missed-tackle-opportunities/blob/main/code/spatial.py

Helper functions for calculating spatial relationships between players
"""
...
def get_positions_from_dataframe(df: pd.DataFrame) -> dict:
    """
...
def team_influence(
    ...
    offensive_influence_fn = multi_player_influence(
        list(offensive_positions), list(offensive_directions), list(offensive_speeds)
    )
    defensive_influence_fn = multi_player_influence(
        list(defensive_positions), list(defensive_directions), list(defensive_speeds)
    )
...
def get_influence_for_frame(frame: pd.DataFrame) -> tuple:
    """
...
def get_influence_for_blockers(frame: pd.DataFrame) -> tuple:
    """
...
def compute_rotation_matrix(direction: float) -> NDArray[np.float64]:
    """
...
def compute_scaling_matrix(speed: float) -> NDArray[np.float64]:
    """
